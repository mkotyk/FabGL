...
#define DRIVE_B BDSC_dsk     // B: read only
#define DRIVE_C "drive_c.dsk"   // C: read/write
#define DRIVE_D "drive_d.dsk"   // D: read/write
...
void emulator_menu(int stream)  // 0 is terminal; 1 is serial
{
  ...
  for (bool loop = true; loop; ) {
    ...
    CONSOLE.write( "\e[93m Z \e[37m Reset\e[K\n\r");
    CONSOLE.write( "\e[93m S \e[37m Send Disk to Serial\e[K\n\r");
    CONSOLE.write( "\e[93m R \e[37m Get Disk from Serial\e[K\n\r");
    CONSOLE.write( "\e[93m F \e[37m Format FileSystem\e[K\n\r");
    CONSOLE.printf("\e[93m U \e[37m CPU: \e[33m%s\e[K\n\r", preferences.getInt("CPU", DefaultCPU) == 1 ? "Z80" : "i8080");
    CONSOLE.printf("\e[93m P \e[37m Real CPU Speed: \e[33m%s\e[K\n\r", preferences.getBool("realSpeed", false) ? "YES" : "NO");
    CONSOLE.write("\e[6A");  // cursor UP
    CONSOLE.printf("\t\t\t\t\t\e[93m T \e[37m Terminal: \e[33m%s\e[K\n\r", TermStr[preferences.getInt("termEmu", DefaultTermIndex)] );
    CONSOLE.printf("\t\t\t\t\t\e[93m K \e[37m Keyboard Layout: \e[33m%s\e[K\n\r", KbdLayStr[preferences.getInt("kbdLay", DefaultKbdLayIndex)] );
    CONSOLE.printf("\t\t\t\t\t\e[93m G \e[37m CRT Mode: \e[33m%s\e[K\n\r", preferences.getBool("emuCRT", false) ? "YES" : "NO");
    CONSOLE.printf("\t\t\t\t\t\e[93m C \e[37m Colors: \e[33m%s\e[K\n\r", ColorsStr[preferences.getInt("colors", DefaultColorsIndex)] );
    CONSOLE.write( "\t\t\t\t\t\e[93m O \e[37m Reset Configuration\e[K\n\r");

    CONSOLE.write("\n\n\e[97mPlease select an option (ENTER to exit): \e[K\e[92m");
    int ch=getChar(CONSOLE);
    ...
    switch (ch) {
      ...
      case 'R':
      {
        CONSOLE.write("Select a drive (A, B, C, D...): ");
        char drive = getChar(CONSOLE);
        if (ch == 'S') {
          CONSOLE.printf("\n\rSending drive %c...\n\r", drive);
      ...
      // Format FileSystem
      case 'F':
        CONSOLE.write("Formatting filesystem removes RW disks and resets the Altair. Are you sure? (Y/N)");
        if (getChar(CONSOLE) == 'Y') {
          CONSOLE.write("\n\rFormatting...");
          CONSOLE.flush();
      ...
      // Reset
      case 'Z':
        CONSOLE.write("Resetting the Altair. Are you sure? (Y/N)");
        if (getChar(CONSOLE) == 'Y')
          ESP.restart();
        break;
  ...
  if (resetRequired) {
    CONSOLE.write("Reset required. Reset now? (Y/N)");
    if (getChar(CONSOLE) == 'Y') {
      diskDrive.detachAll();
      ESP.restart();
    }
...
void loop()
{
  Stream &TerminalX=streamsel?(Stream &)Serial:(Stream &)Terminal;
  TerminalX.write("Initializing filesystem...\r");
  TerminalX.flush();

  if (FileBrowser::mountSDCard(FORMAT_ON_FAIL, SDCARD_MOUNT_PATH))
    basepath = SDCARD_MOUNT_PATH;
  else 
 if (FileBrowser::mountSPIFFS(FORMAT_ON_FAIL, SPIFFS_MOUNT_PATH))
  ...
  TerminalX.write("Creating Disk C...        \r");
  TerminalX.flush();
  diskDrive.attachFile(2, (String(basepath) + String("/") + String(DRIVE_C)).c_str());


  TerminalX.write("Creating Disk D...        \r");
  TerminalX.flush();
...
//  SIO1.attachStream(&Terminal);
  ...
  if (streamsel==0) TerminalX.write("\e[1;24r");  // don't do this for ANSI serial terminal

  TerminalX.write("\e[97m\e[44m");
  TerminalX.write("                    /* * * * * * * * * * * * * * * * * * * *\e[K\r\n");
  TerminalX.write("\e#6");
  TerminalX.write("\e#3              ALTAIR  8800\e[K\r\n");
  TerminalX.write("\e#4              ALTAIR  8800\e[K\r\n");
  TerminalX.write("\e#5");
  TerminalX.write("\e[K\r\n");
  TerminalX.write("                     \e[37mby Fabrizio Di Vittorio - www.fabgl.com\e[97m\e[K\r\n");
  TerminalX.write("                     * * * * * * * * * * * * * * * * * * * */\e[K\r\n\e[K\n");

  TerminalX.printf("\e[33mFree Memory :\e[32m %d bytes\e[K\r\n", heap_caps_get_free_size(0));

  int64_t total, used;
  FileBrowser::getFSInfo(FileBrowser::getDriveType(basepath), 0, &total, &used);
  TerminalX.printf("\e[33mFile System :\e[32m %lld KiB used, %lld KiB free\e[K\r\n", used / 1024, (total - used) / 1024);

  TerminalX.printf("\e[33mKbd Layout  : \e[32m%s\e[K\r\n", KbdLayStr[preferences.getInt("kbdLay", DefaultKbdLayIndex)] );
  TerminalX.printf("\e[33mCPU         : \e[32m%s\e[92m\e[K\r\n\e[K\n", preferences.getInt("CPU", DefaultCPU) == 1 ? "Z80" : "i8080");

  if (streamsel) TerminalX.printf("Press User button to display emulator menu\r\n"); else TerminalX.printf("Press \e[93mPAUSE\e[92m to display emulator menu\e[K\r\n");
